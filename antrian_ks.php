...
// Hapus data pada tabel antrian_karangsatria 1 menit setelah waktu reset berakhir
if ($waktu_sekarang >= ($waktu_reset_end + 60) && $waktu_sekarang < ($waktu_reset_end + 120)) {
    try {
        // Pindahkan data ke tabel log_antrian_karangsatria
        $sql_insert_log = "INSERT INTO log_antrian_karangsatria (device_id, ip_address, random_number, nama, telepon, tanggal)
                           SELECT device_id, ip_address, random_number, nama, telepon, tanggal FROM antrian_karangsatria";
        $pdo->exec($sql_insert_log);
        
        // Hapus data dari tabel antrian_karangsatria
        $sql_delete = "DELETE FROM antrian_karangsatria";
        $pdo->exec($sql_delete);
        
        $message = "Data antrian_karangsatria telah dipindahkan ke log dan dihapus.";
    } catch (PDOException $e) {
...
$total_antrian_karangsatria = 0;
$antrian_karangsatria_list = null;
$message = "";

// Periksa jika perangkat atau IP sudah menghasilkan nomor antrian_karangsatria hari ini
try {
    $sql_check_device = "SELECT * FROM antrian_karangsatria WHERE (device_id = :device_id OR ip_address = :ip_address) AND tanggal = :tanggal";
    ...
    $antrian_karangsatria_hari_ini = $stmt_check_device->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Error checking device: " . $e->getMessage());
}

// Jika perangkat sudah mendapatkan nomor antrian_karangsatria
if ($antrian_karangsatria_hari_ini) {
    $message = "Anda Sudah Mengambil Nomor Antrian";
}

// Hitung total antrian_karangsatria
try {
    $sql_total = "SELECT COUNT(*) AS total_antrian_karangsatria FROM antrian_karangsatria WHERE tanggal = :tanggal";
    $stmt_total = $pdo->prepare($sql_total);
    $stmt_total->bindParam(':tanggal', $tanggal_hari_ini);
    $stmt_total->execute();
    $total_data = $stmt_total->fetch(PDO::FETCH_ASSOC);
    $total_antrian_karangsatria = $total_data['total_antrian_karangsatria'] ?? 0;
} catch (PDOException $e) {
    die("Error menghitung total: " . $e->getMessage());
}

// Ambil daftar antrian_karangsatria hari ini
try {
    $sql_antrian_karangsatria_list = "SELECT random_number, nama FROM antrian_karangsatria WHERE tanggal = :tanggal ORDER BY random_number ASC";
    $stmt_antrian_karangsatria_list = $pdo->prepare($sql_antrian_karangsatria_list);
    $stmt_antrian_karangsatria_list->bindParam(':tanggal', $tanggal_hari_ini);
    $stmt_antrian_karangsatria_list->execute();
    $antrian_karangsatria_list = $stmt_antrian_karangsatria_list->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Error fetching antrian_karangsatria list: " . $e->getMessage());
}

// Proses form untuk mendapatkan nomor antrian_karangsatria baru
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$antrian_karangsatria_hari_ini && $form_aktif) {
    $nama = $_POST['nama'];
    ...
    // Tentukan grup berdasarkan total antrian_karangsatria
    $group_number = floor($total_antrian_karangsatria / 10) + 1;
    ...
    do {
        ...
        try {
            $sql_check = "SELECT COUNT(*) AS count FROM antrian_karangsatria WHERE random_number = :random_number AND tanggal = :tanggal";
    ...
    // Masukkan data antrian_karangsatria baru ke dalam tabel antrian_karangsatria
    try {
        $sql_insert = "INSERT INTO antrian_karangsatria (device_id, ip_address, random_number, nama, telepon, tanggal) 
        ...
        $antrian_karangsatria_hari_ini = ['random_number' => $random_number, 'nama' => $nama];
    } catch (PDOException $e) {
        die("Error memasukkan antrian_karangsatria baru: " . $e->getMessage());
    }
}
...
<body>
    <div class="container">
        ...
        <div class="card">
            ...
            <p>Antrian hari ini: <strong><?= $total_antrian_karangsatria ?></strong></p>

            <?php if (!$form_aktif): ?>
                <p style="color: red; font-weight: bold;">Antrian Belum Dibuka</p>
            <?php elseif ($antrian_karangsatria_hari_ini): ?>
                <div class="antrian_karangsatria-box">
                    <h2>Nomor Antrian Anda:</h2>
                    <p class="antrian_karangsatria-number"><?= $antrian_karangsatria_hari_ini['random_number'] ?></p>
                    <p>Nama: <strong><?= $antrian_karangsatria_hari_ini['nama'] ?></strong></p>
            ...
            <?php if ($total_antrian_karangsatria > 0): ?>
                <table>
                    <thead>
                        <tr>
                            <th>Nomor</th>
                    ...
                    <tbody>
                        <?php foreach ($antrian_karangsatria_list as $row): ?>
                            <tr>
